...
## >>>>>>>>>>>>>>>>>>>>>>>>>>>>> Custom Personal Configuration START <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
...
alias rccat='rgrep -Env "^$|^#|^;|^//"'
alias pcat='pygmentize'

alias gogo='go run'
alias gobg='go build -ldflags "-w -s -H=windowsgui"'
alias gobuild='go build -ldflags "-w -s"'
alias gotowin='go env -w GOOS=windows'
alias goretlx='go env -w GOOS=linux'
...
getlcr() {
    ...
    then
        ...
        echo -e "\033[31;1m[-]\033[0m Error: $lcr"
    fi
}

sslssh() {
   ## server
   # openssl req -new -subj "/CN=news" -days 365 -extensions v3_req -x509 -out tunnel.crt -newkey rsa:2048 -keyout tunnel.key -nodes
   # sudo socat OPENSSL-LISTEN:443,fork,cert=tunnel.crt,key=tunnel.key,verify=0 TCP-CONNECT:localhost:22

   ## client
   # ssh -o ProxyCommand='socat STDIO OPENSSL-CONNECT:%h:443,verify=0' user@ip
}

getv2raya() {
    wget -qO - https://apt.v2raya.org/key/public-key.asc | sudo tee /etc/apt/trusted.gpg.d/v2raya.asc; getlcr;
    echo "deb https://apt.v2raya.org/ v2raya main" | sudo tee /etc/apt/sources.list.d/v2raya.list;     getlcr;
    sudo apt update;                        getlcr;
    sudo apt install v2ray v2raya;          getlcr;

    sudo systemctl start v2raya;            getlcr;
}
...
# apt install v2raya #下载v2raya
# ## 如果启动服务遇到v2ray-core未安装可以执行: apt install v2ray解决问题
...
## >>>>>>>>>>>>>>>>>>>>>>>>>>>>> Custom Personal Configuration END <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
